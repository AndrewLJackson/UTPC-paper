...
do_tpc_fit <- function(x){
  ...
  fit <- optim(get_inits(x), 
               ...
               ta = x$ta,
               method = "L-BFGS-B",
               lower = c(0.001, -50, 0.001, -50), 
               upper = c(Inf, 50, Inf, 50))
  
  # return a tibble (data.frame)
  return(x %<>% mutate(u_yopt  = fit$par[1],
                       u_Topt  = fit$par[2],
                       u_Delta = 1 / fit$par[1],
                       u_Tc    = (1 / fit$par[1]) + fit$par[2],
                       x1 = fit$par[1], 
                       x2 = fit$par[2], 
                       x3 = fit$par[3], 
                       x4 = fit$par[4])
...
## Andrew Testing new model fit

```{r}

aj <- combined_dat_params

aj %<>% mutate(y_i_dat = x3 * (pf - x4),
               x_i_dat =  x1 * (ta - x2) )
...
dd_utpc <- data.frame(x = seq(-50, 1, length.out = 100))
dd_utpc %<>% mutate(y = (1-x) * exp(x))

gg1 <- ggplot(data = aj, mapping = aes(x = x_i_dat, 
                                       y = y_i_dat)) + 
  geom_point() + 
