...
Some of the model fits end up close to boundaries of our set limits on Topt, and we can start by excluding those. In the full dataset there are `r length(unique(type))` experiments reported. We create a new column that flags the dataset as "problematic" if it satifies our criteria. 

```{r}
dd_utpc %<>% mutate(problematic = case_when(
  u_Topt <= 0.1 | u_Topt >99 ~ TRUE, 
  u_yopt >= 5 ~ TRUE,
  .default = FALSE
))
...
type_Tc_Topt <- dd_utpc %>% 
                         filter(!problematic) %>%
  group_by(type) %>% summarise(Topt = u_Topt[1], Tc = u_Tc[1]) %>%
  ungroup()

# there are small proportion of rows with Topt = the lower limit of 
# our model fitting process, which we chose to exclude
type_Tc_Topt %<>% filter(Topt >= 0.01)

gg_Topt_Tc <- ggplot(data = type_Tc_Topt, 
                     mapping = aes(x = Topt, y = Tc)) + 
...
We could plot them but there are `r length(unique(dd_prbs$type))` and thats too many even for a series of multipanel plots. Here we just plot the first page of them.
...
g_raw_prbs <- ggplot(data = dd_prbs %>% 
  ...
  facet_wrap_paginate(vars(type), scales = "free", ncol = 7, nrow = 6,
                      page = 1)
  # scale_color_viridis_d(guide="none")
...
Some of the datasets appear to hit the floor at zero on the UTPC scaled performance axis. Here we apply a crude filter to identify these.

```{r}
# a helper function to find the experiments (column "types") that 
# contain very low performance values
zero_fun <- function(x){ifelse(any(x), 1, 0)}

# make a copy of the main data.frame object
zz_utpc <- dd_utpc

# find which rows have very low performance values for rescaled 
# temperatures larger than -1, which according to UTPC should be 
# close to pf = 1 around the optimum.
zz_utpc$zeros <- ifelse(dd_utpc$ta_utpc >= -1 & 
                              dd_utpc$ta_utpc <= 0 & 
                              dd_utpc$pf_utpc <= 0.001, 1, 0)

# apply a filter code to each experiment (type) that 
...
# plot only these zero-filtered experiments.
