		...
		# Jean-François Arnoldi, Andrew L. Jackson, Ignacio Peralta-Maraver and Nicholas L. Payne
...
# -----------------------------------------------------------------------------
# Second part: refitting thermal performance data using UTPC model. 

# load combined data from Rezende & Bozinovic, 2019
load("combined_dat_params.rda")

# open code to fit TPC using model from Rezende and Bozinovic 2019
source("Analyses_thermal_functions.R")

# create a data.frame with observations and parameters from Rezende and Bozinovic 2019
data <- data.frame(combined_dat_params)[,c(1:5,11:30)] #remove unused columns
head(data)

# Import data files
photo <- subset(data, type == "pho") # Photosynthesis data 
run <- subset(data, type == "run") # Running speed data 
pop <- subset(data, type == "pop") # Population growth rate data 

fitted <- data # Fitted parammeters of TPC

fitted <- ddply(fitted, c("code"), summarise,
type = unique(type), 
q10 = mean(q10),
cte = mean(cte),
thr = mean(thr),
...
max.pf = mean(max.pf),
ta.opt = mean(ta.opt),
breadth.50 = mean(breadth.50),
r.square  = mean(r.square),
n.samples = mean(n.samples),
K.param = mean(K.param),       
AIC = mean(AIC), 
type2 = unique(type2),    
u_yopt = mean(u_yopt),   
u_Topt = mean(u_Topt),
u_Delta = mean(u_Delta))

# Define the universal thermal performance function and restrict it to y>=0
utpc_fun <-  function(x){y <- (1-x) * exp(x)}

# ----------
# Rescale data using Rezende’s model estimates. 
# First, we use the estimated parameters for Topt, Tc and pmax to rescale the data and illustrate its relationship to the universal TPC.

# photosynthesis data
photo$pf_r <- photo$pf/photo$u_yopt
photo$ta_r <- (photo$ta - photo$u_Topt)/(photo$u_Tc - photo$u_Topt)

# Speed run data
run$pf_r <- run$pf/run$u_yopt
run$ta_r <- (run$ta - run$u_Topt)/(run$u_Tc - run$u_Topt)

# Population growth data
pop$pf_r <- pop$pf/pop$u_yopt
pop$ta_r <- (pop$ta - pop$u_Topt)/(pop$u_Tc - pop$u_Topt)
